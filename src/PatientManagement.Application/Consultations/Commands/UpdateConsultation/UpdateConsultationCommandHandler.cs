...
public class UpdateConsultationCommandHandler : IRequestHandler<UpdateConsultationCommand, ConsultationDto>
{
    private readonly IApplicationDbContext _context;

    public UpdateConsultationCommandHandler(IApplicationDbContext context)
    {
        _context = context;
    }

    public async Task<ConsultationDto> Handle(UpdateConsultationCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            throw new KeyNotFoundException($"Consultation with ID {request.Id} not found");
        }

        // Update Vitals if provided
        if (request.Vitals != null)
        {
            consultation.Vitals = new Vitals(
        ...
        consultation.ClinicalNotes = request.ClinicalNotes ?? consultation.ClinicalNotes;
        consultation.TreatmentPlan = request.TreatmentPlan ?? consultation.TreatmentPlan;
        consultation.FollowUpInstructions = request.FollowUpInstructions ?? consultation.FollowUpInstructions;
        
        if (request.NextVisitDate.HasValue)
        {
            consultation.NextVisitDate = request.NextVisitDate;
        }
